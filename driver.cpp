...
struct Table {
    ...
    void generate(string type, string colNametoIndex, bool infunc);
    ...
    if (it != colNames.end()) {
        ...
        if (generateColName != "") {
            if (!searchInxhash.empty()) {
                generate("hash", generateColName, true);
            }
            else if (!searchInxbst.empty()) {
                generate("bst", generateColName, true);
            }
        }
    }
    else {
...
//GENERATE
void Table::generate (string type, string colNametoIndex, bool infunc) {
